...
impl Database {
    ...
    pub(crate) fn create_tables_on_conn(conn: &Connection) -> Result<(), AppError> {
        ...
        conn.execute(
            ...
            enabled_pi_coding_agent BOOLEAN NOT NULL DEFAULT 0
        )",
            [],
        )
        .map_err(|e| AppError::Database(e.to_string()))?;
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    11 => {
                        log::info!("迁移数据库从 v11 到 v12（添加 Pi Agent MCP/Skills 支持）");
    ...
    /// v11 -> v12: 添加 Pi Agent 的 MCP 和 Skills 启用状态列。
    fn migrate_v11_to_v12(conn: &Connection) -> Result<(), AppError> {
        if Self::table_exists(conn, "mcp_servers")? {
            Self::add_column_if_missing(
                conn,
                "mcp_servers",
                "enabled_pi_coding_agent",
                "BOOLEAN NOT NULL DEFAULT 0",
            )?;
        }
        ...
        log::info!("v11 -> v12 迁移完成：已添加 Pi Agent MCP/Skills 支持");
        Ok(())
    }
