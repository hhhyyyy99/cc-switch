...
pub fn sync_single_server_to_pi_coding_agent(
...
pub fn remove_server_from_pi_coding_agent(id: &str) -> Result<(), AppError> {
    if !should_sync_pi_mcp() {
        return Ok(());
    }
...
pub fn import_from_pi_coding_agent(config: &mut MultiAppConfig) -> Result<usize, AppError> {
    ...
    for (id, spec) in mcp_map {
        ...
        if let Some(existing) = servers.get_mut(&id) {
            if !existing.apps.pi_coding_agent {
                existing.apps.pi_coding_agent = true;
                changed += 1;
