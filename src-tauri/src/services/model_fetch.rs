...
fn auth_modes_for_fetch(auth_header: Option<bool>) -> Vec<ModelFetchAuthMode> {
    match auth_header {
        ...
        _ => vec![ModelFetchAuthMode::Bearer, ModelFetchAuthMode::ApiKey],
    }
}
...
mod tests {
    ...
    #[test]
    fn test_auth_modes_default_to_bearer_with_api_key_fallback() {
        let modes = auth_modes_for_fetch(None);

        assert!(matches!(modes[0], ModelFetchAuthMode::Bearer));
        assert!(matches!(modes[1], ModelFetchAuthMode::ApiKey));
    }
